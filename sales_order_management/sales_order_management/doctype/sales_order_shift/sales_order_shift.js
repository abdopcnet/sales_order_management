		...
		try {
			...
			if (!frm.doc.__islocal && frm.doc.name) {
				frm.add_custom_button(__("إعادة فحص المدفوعات"), () => {
					frappe.call({
						method: "custom_reports.custom_reports.doctype.sales_order_shift.sales_order_shift.refresh_payments",
						args: {
						...
						callback: function (r) {
							if (r.message && r.message.status === "success") {
								...
								console.log(
									"[sales_order_shift.js] refresh callback: No success status"
								);
							}
						},
						error: function (err) {
